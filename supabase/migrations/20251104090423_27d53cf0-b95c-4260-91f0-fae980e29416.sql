-- Update sync function to use correct Supabase URL
DROP FUNCTION IF EXISTS sync_to_external_db() CASCADE;
...
DECLARE
  ...
  response_status int;
BEGIN
  -- Use the project's Supabase URL
  function_url := 'https://vbvmlknbqlnmepanptvm.supabase.co/functions/v1/database-sync';
  ...
  -- Log if sync fails (optional)
  IF response_status NOT BETWEEN 200 AND 299 THEN
    RAISE WARNING 'Sync to external DB failed with status: %', response_status;
  END IF;
